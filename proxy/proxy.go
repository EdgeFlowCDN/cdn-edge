...
func NewServer(cfg *config.Config, cacheManager *cache.Manager, accessLogger *cdnlog.AccessLogger) *Server {
	return &Server{
		cfg:          cfg,
		reloader:     NewConfigReloader(cfg.Domains),
...
// Reloader returns the config reloader for external updates (e.g., gRPC sync).
func (s *Server) Reloader() *ConfigReloader { return s.reloader }

// CacheManager returns the cache manager for external purge operations.
func (s *Server) CacheManager() *cache.Manager { return s.cacheManager }
...
func (s *Server) ListenAndServe() error {
	...
	cdnlog.Info("starting edge proxy", "addr", addr)
